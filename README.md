...
- **五级大纲契约**：总纲、部纲、卷纲、幕纲和章纲分别维护草稿与已发布版本；只有已发布并同步的父级计划才能生成下一层或进入正文上下文。
- **结构化创作**：章节节拍、人物设定、世界观、故事线和伏笔都可以围绕同一作品维护，并受已发布大纲约束。
- **持续记忆**：章节完成后沉淀摘要、事件、三元组、伏笔和向量索引，为后续生成提供可追溯的上下文。
- **知识图谱**：从作品设定和章节事实中维护实体关系，支持人物、地点和故事关系的可视化查看。
- **候选审稿与自动驾驶**：支持连续自动驾驶和逐章人工审核；候选正文、机器审校与提交清单均独立于正式书稿，未通过内容不会进入记忆或后续提示词。
- **进度与恢复保护**：工作台会显示候选生成、DAG V2 当前节点、重试、待审核、正式写入、记忆同步、暂停和失败等权威状态；刷新页面后从本地 SQLite 轨迹续接，不依赖模拟动画。
- **世界线重生成**：可从任意章节开始重写，保留前缀不变，将旧尾部归档为只读可恢复世界线，并从保留前缀重建规范事实与长期记忆。
...
3. 在“编辑五级大纲”中按总纲、部纲、卷纲、幕纲、章纲逐级完善计划。总纲先写故事剧情走向、人物成长、冲突与结局方向；下级承接父级和上一同级的结束状态。每一级都先发布并同步，再生成或编辑下一层；未发布的草稿不会进入正文提示词。
4. 回到“全托管驾驶”，选择 **连续自动驾驶** 或 **逐章人工审核**。前者会在正式写入和记忆同步完成后自动推进；后者每章都会停在审稿台，下一章不会预取或额外消耗 Token。
5. 在“独立审稿台”检查候选正文、五级大纲契约、机器审校和提交清单。作者修改正文后需要重新审校，只有最后确认的正文和清单才会写入正式书稿与记忆。
6. 在工作台生成、编辑并保存章节；侧边栏可检查人物、地点、伏笔、关系和章节状态。
...
从工作台“全托管驾驶”打开 **编辑五级大纲**。总纲、部纲、卷纲、幕纲和章纲是同一条文学规划链：总纲描述故事主线，部纲、卷纲、幕纲和章纲不断拆分成时间上承接的阶段经历。先保存草稿，确认后发布并同步；子级生成只读取已发布的父级版本和上一同级的交接状态。这样可以在保留作者手改内容的同时，避免旧计划或未确认草稿混入后续写作。

“叙事骨架”不再是新书的独立规划入口。需要部、卷、幕或章节结构时，请在五级大纲工作室中从已发布父级继续生成，确保整本书只有一条可追溯的规划权威。

AI 流式大纲生成会保存为可恢复的尝试：页面刷新后会恢复已接收文本；失败或取消后可以复用同一份已发布上下文重试。只有结构校验通过的完成尝试才会写入普通大纲草稿。

大纲的后期修改不会静默改写已经发生的正文事实。工作室会给出影响范围；当新计划与历史章节冲突时，应从冲突章节创建世界线重生成，而不是直接覆盖既有记忆。

### 连续与逐章模式

启动自动驾驶前选择一种模式：

- **连续自动驾驶**：候选章通过、正式写入和规范记忆同步完成后，自动继续下一章；硬冲突、记忆失败或技术错误会暂停。
- **逐章人工审核**：系统一次只保留一个候选章。候选待审、作者改稿或审校过期期间，后续章节不会调用 LLM。

两种模式都从候选优先入口启动；旧的直接“全托管”启动入口已禁用，不能绕过候选审稿、正式提交和规范记忆同步。

候选正文会在独立审稿台中展示。作者可以修改正文或提交清单；修改后旧审校会标记为过期，重新审校完成前不能正式提交。未被采纳的候选稿不会进入正式章节、事实记忆、向量索引或下一章上下文。候选生成实际由 DAG V2 执行；节点尝试、失败原因和耗时保存在本地，可在工作台看到真实进度并在刷新后恢复。

### 从任意章重生成

在“从任意章重生成”中输入起始章节 `N` 和新的目标章节数。系统会保留第 `1` 到 `N-1` 章，将旧的 `N` 到最后一章从当前主线撤出并归档为只读世界线；归档可恢复。执行后先从保留前缀重建摘要、事实、伏笔和长期记忆，完成前不会开始新章节。

`N=1` 表示整部正文世界线重开；`N` 等于当前最后一章时只重写最后一章；`N` 大于当前正式章节数时按普通续写处理。
