...
- **进度与恢复保护**：工作台会显示候选生成、待审核、正式写入、记忆同步、暂停和失败等权威状态；规范章后记忆未提交时，自动驾驶不会继续下一章。
...
3. 在“编辑五级大纲”中按总纲、部纲、卷纲、幕纲、章纲逐级完善计划。每一级都先发布并同步，再生成或编辑下一层；未发布的草稿不会进入正文提示词。
...
从工作台“全托管驾驶”打开 **编辑五级大纲**。总纲、部纲、卷纲、幕纲和章纲是同一条约束链：先保存草稿，确认后发布并同步；子级生成只读取已发布的父级版本。这样可以在保留作者手改内容的同时，避免旧计划或未确认草稿混入后续写作。
...
候选正文会在独立审稿台中展示。作者可以修改正文或提交清单；修改后旧审校会标记为过期，重新审校完成前不能正式提交。未被采纳的候选稿不会进入正式章节、事实记忆、向量索引或下一章上下文。
