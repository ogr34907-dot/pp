...
def test_start_development_servers_starts_the_default_api_and_frontend(tmp_path) -> None:
...
def test_start_development_servers_reuses_ready_servers_and_opens_the_frontend(tmp_path) -> None:
    (tmp_path / "frontend").mkdir()
    python_executable = tmp_path / "python.exe"
    python_windowless_executable = tmp_path / "pythonw.exe"
    python_executable.touch()
    ...
    assert calls == [("browser", "http://127.0.0.1:3000/")]


def test_stop_development_servers_stops_the_default_local_listeners() -> None:
    queried_ports: list[int] = []
    stopped: list[int] = []

    def listening_pids(port: int) -> list[int]:
        queried_ports.append(port)
        return {8005: [101], 3000: [202]}[port]
    ...
    assert queried_ports == [8005, 3000]
    assert stopped_processes == [101, 202]
    assert stopped == [101, 202]


def test_stop_development_servers_deduplicates_process_ids_across_ports() -> None:
    queried_ports: list[int] = []
    stopped: list[int] = []
